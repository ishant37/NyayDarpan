...
import applicantPhoto from '../assets/claim_photo.jpeg';
import defaultAadhar from '../assets/aadhar_card.avif';
import {
  UploadCloud,
  FileText,
  ScanLine,
...
// --- Default mock data (no upload/scan) ---
const defaultPattaData = {
  id: generateFraId(),
...
const ScanDoc = () => {
  const [results, setResults] = useState(defaultPattaData);
  const [qrCodeDataURL, setQrCodeDataURL] = useState('');
  const [isHeatmapOpen, setIsHeatmapOpen] = useState(false);
  const [bhuAadhaarId, setBhuAadhaarId] = useState(null);
  const [isGeneratingPDF, setIsGeneratingPDF] = useState(false);
  // Show default aadhar card always
  const aadharPreview = defaultAadhar;
    ...
    <>
      ...
      <div className="bg-gray-50 min-h-screen p-4 sm:p-6 lg:p-8">
        ...
        <main className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-5xl mx-auto">
          {/* Left Panel: Default Patta & Aadhaar */}
          <Card>
            <CardHeader>
                <CardTitle><UploadCloud className="w-5 h-5 text-gray-400" /> Patta & Aadhaar Card</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-col items-center justify-center text-center">
                <div className="mb-4 overflow-hidden rounded-lg border border-gray-200">
                  <img src={results.applicantImage} alt="Patta" className="max-h-80 w-full object-contain" />
                </div>
                <div className="mb-4 overflow-hidden rounded-lg border border-gray-200">
                  <img src={aadharPreview} alt="Aadhaar Card" className="max-h-80 w-full object-contain" />
                </div>
              </div>
            </CardContent>
          </Card>
                ...
                <div className="border-t pt-6">
                  {!bhuAadhaarId ? (
                      <div className="text-center">
                          ...
                          <img src={aadharPreview} alt="Aadhaar Preview" className="max-h-32 w-auto mx-auto object-contain mb-2 rounded-lg border" />
                          <Button onClick={handleLinkAadhar} className="w-full mt-4"><LinkIcon className="mr-2 h-4 w-4" /> Link & Generate Bhu-Aadhaar</Button>
                      </div>
                  ) : (
                      <div className="text-center">
