...
// Function to generate colors based on area and index
function getPattalandColor(index, area) {
  const colors = [
    '#4CAF50', '#2196F3', '#FF9800', '#9C27B0', 
    '#F44336', '#00BCD4', '#8BC34A', '#FF5722',
    '#3F51B5', '#E91E63', '#009688', '#CDDC39'
  ];
...
const PattaAtlasView = () => {
  ...
  const [mapView, setMapView] = useState('terrain'); // terrain, satellite, hybrid
  const [showPattaLabels, setShowPattaLabels] = useState(true);
  const [selectedPatta, setSelectedPatta] = useState(null);
  const [mapCenter, setMapCenter] = useState({ lat: 23.5, lng: 78.0 });
    ...
    <Box sx={{ position: 'absolute', top: 16, right: 16, zIndex: 1000 }}>
      <Paper elevation={3} sx={{ p: 1, display: 'flex', flexDirection: 'column', gap: 1 }}>
        ...
        <Tooltip title="Center Map">
          <IconButton 
            size="small" 
            onClick={() => setMapCenter({ lat: 23.5, lng: 78.0 })}
          >
            <MyLocation />
          </IconButton>
        </Tooltip>
